...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your Bruut Server account.<br>
        ...
        &mdash; Bruut Server Team
      `
      return sendMail({ toEmail: email, subject: 'Bruut Server - Password Reset', content })
    })
